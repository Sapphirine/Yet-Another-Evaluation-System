...
setwd("/home/leetz/projects/pogo")
...
plot_histogram<-function(r_stats) {
  ...
  for (i in 1:length(r_stats)) {
    cat(r_stats[[i]]$text)
    usableText=str_replace_all(r_stats[[i]]$text,"[^[:graph:]]", " ") 
    cat(usableText)
    emotionVec[i] <- sentiment(usableText)
  }
  #plot the histogram
...
get_NYTdata<-function(qString) {
  nyArts <- system(paste("python api_nytimes.py ",
